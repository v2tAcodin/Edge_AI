...
const practiceExercises = [
    ...
    // =========================================================================
    {
        ...
        docSnippet: "ESP32-S3 TRM §5.3.3: 'GPIO_OUT_W1TS_REG (0x60004008): Writing 1 to bit n in this register sets the corresponding GPIO output bit to HIGH. Writing 0 has no effect.'",
        ...
        example: `Trích xuất ESP32-S3 TRM Ch.5: Ban đầu reg = 0x00 (Thanh ghi GPIO_OUT_W1TS_REG rỗng, các chân LOW). Bật chân GPIO3 nối cảm biến IMU (bit_pos = 3) -> Output = 0x00 | (1 << 3) = 8 (0x08 = 0b00001000). Chân GPIO3 được cấp nguồn, các chân khác giữ nguyên.`,
        hint: `Áp dụng MISRA C:2012 Rule 10.1 (Toán tử bit trên số nguyên không dấu): <code>reg | (1U << bit_pos)</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §11.5: 'TIMERG0_INT_CLR_REG (0x6001F07C): Set bit 0 to clear the Timer 0 interrupt pending status flag. Failure to clear leaves ISR loop locked.'",
        standardRef: "ESP32-S3 TRM Ch.5 / MISRA C:2012 Rule 10.1",
        bookId: "book_esps3_trm",
        ...
        example: `Trích xuất ESP32-S3 TRM Ch.11: Thanh ghi trạng thái cờ ngắt reg = 0xFF (Cả 8 cờ ngắt đang active). Xóa cờ ngắt Timer 0 tại bit 0 (bit_pos = 0) -> Output = 0xFF & ~(1 << 0) = 254 (0xFE = 0b11111110). Cờ Timer 0 đã tắt, các cờ khác được bảo toàn an toàn.`,
        hint: `Sử dụng toán tử AND với bit đảo (Bitwise Inversion): <code>reg & ~(1U << bit_pos)</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ISO C11 Standard §6.5.11: 'The result of the ^ operator is the bitwise exclusive OR of the operands... If corresponding bits differ, result is 1; otherwise 0.'",
        ...
        example: `Trích xuất ISO/IEC 9899:2011: Đảo trạng thái chân đèn Heartbeat LED: Thanh ghi reg = 0xAA (0b10101010). Đảo trạng thái chân GPIO1 (bit_pos = 1) -> Output = 0xAA ^ (1 << 1) = 168 (0xA8 = 0b10101000). Đèn LED chuyển từ sáng sang tắt trong 1 chu kỳ ALU.`,
        hint: `Sử dụng toán tử XOR: <code>reg ^ (1U << bit_pos)</code>.`,
    ...
    },
    {
        ...
        docSnippet: "MISRA C:2012 Rule 12.2: 'The right hand operand of a shift operator shall lie between zero and one less than the width in bits of the essential type.'",
        ...
        example: `Trích xuất MISRA C:2012 & ESP32-S3 TRM: Đọc cờ UART FIFO Full trong thanh ghi UART_STATUS reg = 0x14 (0b00010100). Kiểm tra bit 2 (FIFO Full Flag, bit_pos = 2) -> (0x14 >> 2) & 1 = 1 (Trạng thái TRUE - FIFO đang đầy, cần dừng truyền).`,
        hint: `Dịch phải và kiểm tra bit thấp nhất: <code>(reg >> bit_pos) & 1</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §1.3.2: 'Xtensa LX7 Vector Instructions (128-bit SIMD) require memory addresses to be 16-byte aligned. Unaligned accesses trigger an illegal instruction or memory exception.'",
        ...
        example: `Trích xuất ESP32-S3 TRM Ch.1: Căn lề mảng trọng số Tensor: Kích thước dữ liệu mảng nơ-ron size = 25 bytes. Căn lề lên bội số 16 gần nhất -> Output = 32 bytes (Thêm 7 bytes padding để chia hết cho 16, đảm bảo nạp được vào lệnh SIMD EE.VLD.128).`,
        hint: `Công thức căn lề 16-byte: <code>(size + 15) & ~15</code>.`,
    ...
    },
    {
        ...
        docSnippet: "MISRA C:2012 Rule 21.3: 'The memory allocation and deallocation functions of <stdlib.h> (malloc, calloc, realloc, free) shall not be used in critical systems.'",
        ...
        example: `Trích xuất SEI CERT C MEM31-C & MISRA C: Cần đệm các gói tin cảm biến mỗi gói data_size = 48 bytes, kích thước phần header quản trị block là header_size = 8 bytes -> Tổng kích thước mỗi khối Memory Pool = 48 + 8 = 56 bytes. Phân bổ cố định trong mảng tĩnh BSS.`,
        hint: `Tính tổng kích thước phần dữ liệu và header quản lý: <code>data_size + header_size</code>.`,
    ...
    },
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.8: 'The Tensor Arena holds all inputs, outputs, and intermediate activation arrays. Memory is allocated linearly with head and tail offsets to eliminate fragmentation.'",
        ...
        example: `Trích xuất TinyML (O'Reilly) Ch.8: Offset hiện tại của con trỏ arena là current_offset = 128 bytes. Lớp mạng tích chập kế tiếp yêu cầu kích thước tensor_size = 64 bytes -> Offset mới = 128 + 64 = 192 bytes trong Tensor Arena tĩnh.`,
        hint: `Cộng dồn offset cấp phát tuần tự: <code>current_offset + tensor_size</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Expert C Programming Ch.4: 'Network protocols and CAN bus frames use Big-Endian (MSB first), whereas x86 and Xtensa LX7 are Little-Endian (LSB first). Byte swapping is mandatory.'",
        ...
        example: `Trích xuất Expert C Programming Ch.4 & ISO 11898-1: Nhận được từ 16-bit val = 0x1234 (Big-Endian). Hoán đổi thứ tự byte sang Little-Endian -> Output = 0x3412 ((0x1234 >> 8) | (0x1234 << 8) & 0xFFFF). Tái tạo đúng giá trị cảm biến.`,
        hint: `Dịch byte cao xuống thấp và byte thấp lên cao: <code>((val >> 8) & 0xFF) | ((val & 0xFF) << 8)</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ISO C11 §6.7.2.1: 'There may be unnamed padding within a structure or at its end to achieve proper alignment of members for the target architecture.'",
        ...
        example: `Trích xuất ISO/IEC 9899:2011 §6.7.2.1: Khai báo struct { char id; int value; } trên CPU 32-bit. Kích thước biến id = 1 byte, căn lề mong muốn alignment = 4 bytes -> Số byte padding cần chèn vào giữa = 4 - (1 % 4) = 3 bytes (Tổng kích thước struct là 8 bytes thay vì 5 bytes).`,
        hint: `Nếu size % alignment == 0 thì padding = 0; ngược lại padding = alignment - (size % alignment).`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §2.2: 'Internal SRAM1 and SRAM2 span from 0x3FC88000 to 0x3FCFFFFF (approx 512KB). Addresses in this range support zero-wait-state 240MHz single-cycle access.'",
        ...
        example: `Trích xuất ESP32-S3 TRM Ch.2: Kiểm tra con trỏ Tensor addr = 0x3FCA0000. So khớp dải [0x3FC88000, 0x3FD00000) -> Output = 1 (TRUE - Thuộc SRAM nội bộ tốc độ cao, an toàn cho DMA và AI).`,
        hint: `Kiểm tra điều kiện: <code>addr >= 0x3FC88000 && addr < 0x3FD00000</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim & Schafer Ch.9: 'The Cooley-Tukey decimation-in-time FFT requires bit-reversed addressing of input sequences to enable in-place butterfly computation.'",
        ...
        example: `Trích xuất Oppenheim & Schafer Ch.9: Đảo bit chỉ số FFT 8 điểm: Chỉ số k = 1 (nhị phân 0b001 trên 3-bit). Đảo ngược bit -> 0b100 = 4. Điểm mẫu x[1] hoán đổi với x[4]. Mở rộng 8-bit: byte = 0x80 (0b10000000) -> Output = 0x01 (0b00000001).`,
        hint: `Duyệt qua 8 bit hoặc đảo bit bằng dịch bit và phép OR.`,
    ...
    },
    {
        ...
        docSnippet: "SEI CERT C §MEM31-C: 'Heap fragmentation occurs when free memory is broken into small non-contiguous blocks. Total free memory may be large while allocation of a moderate block fails.'",
        ...
        example: `Trích xuất SEI CERT C Rule MEM31-C: Tổng dung lượng trống total_free = 100,000 bytes, nhưng khối nhớ liên tục lớn nhất có thể cấp phát chỉ là largest_free_block = 40,000 bytes -> Tỷ lệ phân mảnh = 100 * (1 - 40000/100000) = 60%. Cảnh báo nguy cơ OOM!`,
        hint: `Công thức: <code>(int)((1.0 - (double)largest_free / total_free) * 100)</code>.`,
    ...
    // =========================================================================
    {
        ...
        docSnippet: "ESP32-S3 TRM §11.5: 'TIMERG0_T0INT_RAW_REG (0x6001F074): Bit 0 represents the raw interrupt status of Timer 0.'",
        standardRef: "ESP32-S3 TRM Ch.11 GPTimer Group / ISR Pending Status",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP32-S3 TRM Ch.11: Thanh ghi RAW_INT reg = 0x01 (Bit 0 = 1). Kiểm tra cờ Timer 0 (bit_pos = 0) -> Output = 1 (Ngắt Timer 0 vừa kích hoạt, cần chạy hàm phục vụ ISR).`,
        hint: `Kiểm tra bit: <code>(reg >> bit_pos) & 1</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.6: 'An ISR should be as short as possible. Use a counter or semaphore to defer heavy signal processing to a lower-priority RTOS task.'",
        standardRef: "FreeRTOS Kernel Book Ch.6 / Deferred Interrupt Handling",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.6: Cần tích lũy threshold = 10 mẫu trước khi đánh thức Task AI. Bộ đếm hiện tại count = 9. Tăng thêm 1 mẫu -> count = 10 >= threshold -> Đạt ngưỡng đánh thức Task (Output = 1), reset bộ đếm về 0.`,
        hint: `Nếu count + 1 >= threshold thì trả về 1, ngược lại trả về 0.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim DSP Ch.4: 'The sampling period T is the reciprocal of the sampling frequency Fs: T = 1 / Fs seconds = 1,000,000 / Fs microseconds.'",
        standardRef: "ESP32-S3 TRM Ch.11 / Deterministic Sampling Rate",
        bookId: "book_esps3_trm",
        example: `Trích xuất Oppenheim DSP Ch.4 & TinyML: Tần số lấy mẫu micro là freq_hz = 16,000 Hz -> Chu kỳ lấy mẫu micro-giây = 1,000,000 / 16,000 = 62 µs. Nạp giá trị 62 vào thanh ghi báo động của GPTimer.`,
        hint: `Công thức: <code>1000000 / freq_hz</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §11.2: 'The 16-bit prescaler divides the source clock (80MHz APB Clock) to drive the 54-bit counter: f_timer = f_apb / prescaler.'",
        standardRef: "ESP32-S3 TRM Ch.11 Section 11.2 / Clock Prescaler",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP32-S3 TRM Ch.11: Xung nguồn clock_hz = 80,000,000 Hz (80MHz). Muốn bộ đếm đếm với tần số target_hz = 1,000,000 Hz (1 tick = 1 µs) -> Hệ số chia Prescaler = 80,000,000 / 1,000,000 = 80.`,
        hint: `Công thức: <code>clock_hz / target_hz</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §11.3: 'When auto-reload is enabled and counter reaches alarm_val, hardware automatically loads reload_val into the counter register.'",
        standardRef: "ESP32-S3 TRM Ch.11 Section 11.3 / Auto-Reload Mechanism",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP32-S3 TRM Ch.11: Bộ đếm hiện tại current_ticks = 1000, ngưỡng báo động alarm_val = 1000. Vì current_ticks >= alarm_val, phần cứng tự động nạp lại giá trị reload_val = 0 -> Output = 0.`,
        hint: `Nếu current_ticks >= alarm_val thì trả về reload_val; ngược lại trả về current_ticks.`,
    ...
    },
    {
        ...
        docSnippet: "SEI CERT C ARR30-C: 'Circular ring buffers must wrap index using modulo arithmetic to prevent accessing memory outside allocated bounds.'",
        standardRef: "SEI CERT C ARR30-C / Lock-Free Ring Buffer",
        bookId: "book_sei_cert_c",
        example: `Trích xuất SEI CERT C Rule ARR30-C: Vòng đệm có dung lượng capacity = 100 mẫu. Vị trí ghi hiện tại head = 99. Ghi thêm 1 mẫu -> Vị trí ghi kế tiếp = (99 + 1) % 100 = 0 (Quay vòng về đầu mảng an toàn).`,
        hint: `Công thức: <code>(head + 1) % capacity</code>.`,
    ...
    },
    {
        ...
        docSnippet: "SEI CERT C ARR30-C: 'Head and tail pointers advance independently in lock-free single-producer single-consumer configurations.'",
        standardRef: "SEI CERT C ARR30-C / Single-Producer Single-Consumer Buffer",
        bookId: "book_sei_cert_c",
        example: `Trích xuất SEI CERT C Rule ARR30-C: Vòng đệm có capacity = 64. Vị trí đọc hiện tại tail = 63. Đọc xong 1 mẫu -> Vị trí đọc kế tiếp = (63 + 1) % 64 = 0.`,
        hint: `Công thức: <code>(tail + 1) % capacity</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.3: 'Tick counts are stored in unsigned 32-bit integers. Wrap-around arithmetic: (current_tick - start_tick) >= timeout_ticks correctly handles overflow.'",
        standardRef: "Mastering FreeRTOS Real Time Kernel Ch.3 / Tick Management",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.3: Thời điểm bắt đầu start_tick = 100, tick hiện tại current_tick = 250, thời gian chờ tối đa timeout = 100. Khoảng thời gian trôi qua = 250 - 100 = 150 >= 100 -> Output = 1 (TRUE - Đã hết hạn timeout, cần hủy giao tiếp để tránh treo hệ thống).`,
        hint: `Kiểm tra điều kiện: <code>(current_tick - start_tick) >= timeout</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM Ch.5: 'GPIO hardware deglitch and software debounce counters eliminate false transitions caused by mechanical contact bounce.'",
        standardRef: "ESP32-S3 TRM Ch.5 / GPIO Deglitch & Debounce",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP32-S3 TRM Ch.5: Ngưỡng lọc yêu cầu debounce_limit = 5 chu kỳ ổn định. Mức logic đọc được pin_state = 1, đếm hiện tại count = 4. Tăng lên count = 5 >= limit -> Xác nhận phím đã được nhấn thực sự (Output = 1).`,
        hint: `Nếu pin_state == 1 thì tăng count. Nếu count >= debounce_limit trả về 1, ngược lại trả về 0.`,
    ...
    },
    {
        ...
        docSnippet: "MLPerf Tiny (NeurIPS 2021) §4: 'Latency jitter is defined as the absolute deviation between observed execution time and nominal expected period: |t_actual - t_expected|.'",
        standardRef: "MLPerf Tiny Benchmark (NeurIPS 2021) / Latency Profiling",
        bookId: "book_mlperf_tiny",
        example: `Trích xuất MLPerf Tiny (NeurIPS 2021): Chu kỳ ngắt kỳ vọng expected_us = 1000 µs (1ms). Thời gian thực tế đo được qua GPTimer là actual_us = 1003 µs -> Độ lệch Jitter = |1003 - 1000| = 3 µs. Đạt tiêu chuẩn thời gian thực khắt khe.`,
        hint: `Sử dụng hàm trị tuyệt đối: <code>abs(actual_us - expected_us)</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §11.3: 'Hardware alarm comparators match counter value with TIMERG0_T0ALARM_REG without software intervention.'",
        standardRef: "ESP32-S3 TRM Ch.11 / Hardware Alarm Comparator",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP32-S3 TRM Ch.11: Chu kỳ báo động period = 500 ticks. Bộ đếm hiện tại counter = 1500. Vì 1500 chia hết cho 500 (counter % period == 0) -> Kích hoạt ngắt chu kỳ (Output = 1).`,
        hint: `Kiểm tra chia hết: <code>counter % period == 0 ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.6: 'ISR duration must not exceed critical deadline threshold (e.g. 50us) to maintain system responsiveness.'",
        standardRef: "FreeRTOS Kernel Book Ch.6 / MISRA C:2012 ISR Execution Limit",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.6: Ngưỡng an toàn tối đa cho phép là max_allowed_us = 50 µs. Thời gian đo được của ISR hiện tại là isr_exec_us = 65 µs -> Vi phạm ngưỡng (Output = 1), gửi cảnh báo để kỹ sư tối ưu hóa code ISR.`,
        hint: `Nếu isr_exec_us > max_allowed_us trả về 1; ngược lại trả về 0.`,
    ...
    // =========================================================================
    {
        ...
        docSnippet: "MPU-6050 Datasheet §4.17: 'Accelerometer measurements are output as 16-bit 2s complement values stored in two consecutive registers: ACCEL_XOUT_H (MSB) and ACCEL_XOUT_L (LSB).'",
        standardRef: "ESP32-S3 TRM Ch.26 I2C Master / Sensor Data Merging",
        bookId: "book_esps3_trm",
        example: `Trích xuất MPU-6050 Datasheet & ESP32-S3 TRM Ch.26: Đọc từ cảm biến I2C byte cao msb = 0x1A (26) và byte thấp lsb = 0x2B (43) -> Ghép thành từ 16-bit: (0x1A << 8) | 0x2B = 0x1A2B = 6699.`,
        hint: `Công thức: <code>(msb << 8) | lsb</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ISO C11 §6.2.6.2: 'Signed integer types represent negative values using twos complement. If the sign bit (bit 15) is 1, the value is negative: val - 65536.'",
        standardRef: "ISO/IEC 9899:2011 §6.2.6.2 / Two's Complement Sign Extension",
        bookId: "book_iso_c11_standard",
        example: `Trích xuất ISO/IEC 9899:2011 §6.2.6.2: Cảm biến xuất từ 16-bit không dấu raw_val = 0xFFF0 (65520). Bit 15 là 1 -> Giá trị có dấu thực tế là 65520 - 65536 = -16 (Cảm biến đang gia tốc ngược chiều).`,
        hint: `Nếu raw_val >= 32768 thì trả về raw_val - 65536, ngược lại trả về raw_val.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim & Schafer Ch.7: 'The M-point moving average filter has impulse response h[n] = 1/M for 0 <= n < M. It acts as an optimal linear smoother for white noise reduction.'",
        standardRef: "Oppenheim & Schafer Ch.7 / Moving Average Filter",
        bookId: "book_oppenheim_dsp",
        example: `Trích xuất Oppenheim & Schafer Ch.7: 3 mẫu gia tốc liên tiếp là s0 = 10, s1 = 14, s2 = 12 -> Trung bình trượt y = (10 + 14 + 12) / 3 = 36 / 3 = 12. Nhiễu gai đã được làm phẳng.`,
        hint: `Tính trung bình cộng 3 số nguyên: <code>(s0 + s1 + s2) / 3</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim DSP: 'Median filtering is a non-linear digital filtering technique, often used to remove impulse noise (spikes) while preserving sharp edges.'",
        standardRef: "Discrete-Time Signal Processing (Oppenheim) / Nonlinear Median Filter",
        bookId: "book_oppenheim_dsp",
        example: `Trích xuất Oppenheim & Schafer DSP: 3 mẫu cảm biến liên tiếp [10, 100, 12] (mẫu 100 là gai nhiễu do tia lửa điện). Sắp xếp theo thứ tự: [10, 12, 100] -> Giá trị trung vị ở giữa là 12. Gai nhiễu 100 bị triệt tiêu hoàn toàn!`,
        hint: `Sắp xếp hoặc so sánh tìm phần tử nằm ở giữa 3 số a, b, c.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim DSP Ch.7: 'DC blocking is achieved by subtracting the mean signal level: y[n] = x[n] - DC_Offset, centering the waveform at zero before spectral analysis.'",
        standardRef: "ESP32-S3 TRM Ch.29 I2S DMA / Digital Audio DC Blocking",
        bookId: "book_oppenheim_dsp",
        example: `Trích xuất ESP32-S3 TRM Ch.29 & Oppenheim DSP: Mẫu âm thanh nhận được sample = 520, độ lệch một chiều trung bình dc_offset = 512 -> Mẫu sau khi khử DC = 520 - 512 = 8.`,
        hint: `Công thức: <code>sample - dc_offset</code>.`,
    ...
    },
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.7: 'Feature normalization maps raw sensor signals from [min, max] to a standard range [0, target_scale] to ensure uniform neural network gradient flow.'",
        standardRef: "TinyML (O'Reilly) Ch.7 / Feature Normalization Min-Max",
        bookId: "book_tinyml_oreilly",
        example: `Trích xuất TinyML (O'Reilly) Ch.7: Giá trị đo val = 50, dải đo min = 0, max = 100, thang đo mục tiêu target_scale = 255 -> Normalized = (50 - 0) * 255 / (100 - 0) = 127.`,
        hint: `Công thức: <code>(val - min_val) * target_scale / (max_val - min_val)</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Jacob et al. 2018 CVPR §2: 'Clamp function: clamp(x, a, b) = min(max(x, a), b) restricts activations to the representable range of fixed-point containers.'",
        standardRef: "Benoit Jacob et al. 2018 CVPR / Clamping Function",
        bookId: "book_jacob_quantization",
        example: `Trích xuất Benoit Jacob et al. 2018 CVPR: Ngưỡng cho phép từ low = -100 đến high = 100. Tín hiệu đầu vào val = 150 -> Bị cắt ngọn xuống 100; nếu val = -120 -> Cắt lên -100; nếu val = 45 -> Giữ nguyên 45.`,
        hint: `Nếu val < low trả về low; nếu val > high trả về high; ngược lại trả về val.`,
    ...
    },
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.10: 'Vector magnitude squared |a|^2 = ax^2 + ay^2 + az^2 captures total kinetic energy without the latency of computing square roots.'",
        standardRef: "TinyML (O'Reilly) Ch.10 / 3-Axis IMU Magnitude",
        bookId: "book_tinyml_oreilly",
        example: `Trích xuất TinyML (O'Reilly) Ch.10: Cảm biến IMU đo được gia tốc x = 3, y = 4, z = 0 -> Bình phương độ lớn = 3^2 + 4^2 + 0^2 = 9 + 16 + 0 = 25.`,
        hint: `Công thức: <code>x*x + y*y + z*z</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim DSP §7.1: 'Single-pole IIR filter (EMA): y[n] = alpha * x[n] + (1 - alpha) * y[n-1]. In fixed-point integer arithmetic: y = (alpha * x + (100 - alpha) * y_prev) / 100.'",
        standardRef: "Oppenheim & Schafer Ch.7 / First-Order IIR Exponential Filter",
        bookId: "book_oppenheim_dsp",
        example: `Trích xuất Oppenheim & Schafer DSP Ch.7: Hệ số trọng số alpha = 20 (20%), mẫu mới current = 100, mẫu lọc trước đó prev = 50 -> Output = (20 * 100 + 80 * 50) / 100 = (2000 + 4000) / 100 = 60.`,
        hint: `Công thức: <code>(alpha * current + (100 - alpha) * prev) / 100</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ISO 10816-1: 'Peak-to-peak displacement/vibration value is the difference between maximum positive and maximum negative peaks within a measurement window.'",
        standardRef: "ISO 10816 Mechanical Vibration Standards / Peak-to-Peak",
        bookId: "book_oppenheim_dsp",
        example: `Trích xuất ISO 10816: Trong cửa sổ 256 mẫu cảm biến rung động quạt gió, giá trị cực đại max_val = 85 và cực tiểu min_val = -40 -> Biên độ Peak-to-Peak = 85 - (-40) = 125.`,
        hint: `Công thức: <code>max_val - min_val</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim DSP Ch.10: 'Zero-Crossing Rate (ZCR) measures the rate at which a signal changes from positive to zero/negative or vice versa, distinguishing voiced and unvoiced audio.'",
        standardRef: "Oppenheim & Schafer Ch.10 / Zero Crossing Rate (ZCR)",
        bookId: "book_oppenheim_dsp",
        example: `Trích xuất Oppenheim & Schafer Ch.10: Chuỗi 4 mẫu âm thanh [10, -5, -3, 8]. Từ 10 sang -5 (đổi dấu lần 1), từ -3 sang 8 (đổi dấu lần 2) -> Tổng số lần đổi dấu qua trục zero = 2.`,
        hint: `Duyệt mảng, kiểm tra tích hai phần tử liền kề <code>x[i] * x[i-1] < 0</code> hoặc đổi dấu.`,
    ...
    },
    {
        ...
        docSnippet: "Oppenheim DSP Ch.9: 'Each frequency bin k of an N-point DFT corresponds to a physical analog frequency: f_k = (k * Fs) / N Hz.'",
        standardRef: "Oppenheim & Schafer Ch.9 / Nyquist-Shannon Sampling Theorem",
        bookId: "book_oppenheim_dsp",
        example: `Trích xuất Oppenheim DSP Ch.9: Tần số lấy mẫu fs = 16,000 Hz, số điểm FFT fft_size = 512, thùng phổ đỉnh bin_k = 32 -> Tần số thực tế = (32 * 16,000) / 512 = 1,000 Hz (Âm thanh tần số 1 kHz).`,
        hint: `Công thức: <code>(bin_k * fs) / fft_size</code>.`,
    ...
    // =========================================================================
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.9: 'In SMP FreeRTOS on dual-core ESP32-S3, xTaskCreatePinnedToCore() binds tasks: Core 0 (Protocol stack / Wi-Fi) and Core 1 (Application / AI Inference).'",
        standardRef: "Mastering FreeRTOS Real Time Kernel Ch.9 / SMP Multi-Core",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.9: Tác vụ mạng task_type = 0 ghim vào Core 0; tác vụ TinyML task_type = 1 ghim vào Core 1 -> Input task_type = 1 -> Output = 1 (Core 1).`,
        hint: `Kiểm tra loại task và trả về chỉ số core 0 hoặc 1.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.4: 'uxQueueSpacesAvailable() returns the number of free spaces available in a queue, preventing producer task blocking when buffers near capacity.'",
        standardRef: "FreeRTOS Kernel Book Ch.4 / Queue Space Monitoring",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.4: Hàng đợi có tổng dung lượng queue_len = 10 phần tử, hiện đang chứa items_in_queue = 7 phần tử -> Số khe trống còn lại = 10 - 7 = 3.`,
        hint: `Công thức: <code>queue_len - items_in_queue</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.3: 'Preemptive scheduler executes the ready task with the highest numerical priority value.'",
        standardRef: "FreeRTOS Kernel Book Ch.3 / Task Priority Hierarchy",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.3: Mức ưu tiên của tác vụ sự cố an toàn là task_prio = 5, mức ưu tiên tham chiếu là threshold = 3. Vì 5 > 3 -> Output = 1 (TRUE - Tác vụ ưu tiên cao, được điều phối thực thi trước).`,
        hint: `Kiểm tra điều kiện: <code>task_prio > threshold ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.5: 'Event groups allow tasks to wait for combinations of events. Each bit in the EventBits_t represents a unique system event status.'",
        standardRef: "FreeRTOS Kernel Book Ch.5 / Event Groups Bitwise Sync",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.5: Cờ bit hiện tại event_bits = 0x07 (0b00000111). Cần kiểm tra cờ BIT_MQTT_CONNECTED (bit_pos = 2) -> (0x07 >> 2) & 1 = 1 (MQTT đã kết nối sẵn sàng).`,
        hint: `Đọc bit trạng thái: <code>(event_bits >> bit_pos) & 1</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.4: 'Circular buffers provide thread-safe queuing when capacity is verified before insertion.'",
        standardRef: "FreeRTOS Kernel Book Ch.4 / Thread-Safe Ring Buffer",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.4: Vòng đệm có capacity = 16, hiện đang chứa count = 16 phần tử -> Output = 1 (TRUE - Vòng đệm đã đầy, Producer phải tạm dừng đẩy mẫu để tránh ghi đè).`,
        hint: `Kiểm tra: <code>count >= capacity ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP-IDF Programming Guide: 'Task Watchdog Timer (TWDT) triggers system reset if registered tasks fail to feed the watchdog within the configured timeout period.'",
        standardRef: "MISRA C:2012 Rule 14.2 / ESP-IDF Task Watchdog Timer (TWDT)",
        bookId: "book_misra_c",
        example: `Trích xuất ESP-IDF TWDT & MISRA C:2012 Rule 14.2: Thời gian cho ăn lần cuối last_feed = 100 ticks, thời điểm hiện tại now = 350 ticks, giới hạn timeout = 200 ticks. Khoảng thời gian không cho ăn = 350 - 100 = 250 > 200 -> Kích hoạt cảnh báo treo Task (Output = 1).`,
        hint: `Kiểm tra: <code>(now - last_feed) > timeout ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "SEI CERT C Rule ARR30-C: 'Ring buffer occupancy calculation must handle index wrap-around: (head - tail + capacity) % capacity.'",
        standardRef: "SEI CERT C Rule ARR30-C / Buffer Index Accounting",
        bookId: "book_sei_cert_c",
        example: `Trích xuất SEI CERT C Rule ARR30-C: Dung lượng buffer_size = 10. Con trỏ ghi head = 2, con trỏ đọc tail = 8. Vì head < tail (đã quay vòng qua 0) -> Số phần tử = (head - tail + buffer_size) % buffer_size = (2 - 8 + 10) % 10 = 4 mẫu.`,
        hint: `Công thức: <code>(head - tail + buffer_size) % buffer_size</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.3: 'uxTaskGetStackHighWaterMark() returns the minimum amount of remaining stack space that has been available to the task since execution began.'",
        standardRef: "Mastering FreeRTOS Real Time Kernel Ch.3 / Stack High Watermark",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.3: Giới hạn an toàn tối thiểu warning_threshold = 256 bytes. Dung lượng Stack chưa dùng đo được là watermark = 128 bytes. Vì 128 < 256 -> Phát tín hiệu cảnh báo nguy hiểm (Output = 1).`,
        hint: `Kiểm tra: <code>watermark < warning_threshold ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.3: 'pdMS_TO_TICKS(xTimeInMs) macro converts time in milliseconds to time in ticks using configTICK_RATE_HZ: ticks = (ms * configTICK_RATE_HZ) / 1000.'",
        standardRef: "FreeRTOS Kernel Book Ch.3 / Macro pdMS_TO_TICKS Conversion",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.3: Muốn delay ms = 500 mili-giây, hệ điều hành cấu hình tick_rate_hz = 1000 Hz -> Số Tick cần truyền = (500 * 1000) / 1000 = 500 ticks. Nếu tick_rate_hz = 100 Hz -> Số Tick = 50 ticks.`,
    ...
    },
    {
        ...
        docSnippet: "NASA JPL Report (1997): 'Unbounded Priority Inversion occurred when meteorological task (low) held 1553 bus mutex, preempted by communications task (medium), starving the high-priority attitude control task. Mutex Priority Inheritance solved the anomaly.'",
        standardRef: "Mastering FreeRTOS Ch.7 / NASA Mars Pathfinder Priority Inversion",
        bookId: "book_freertos_kernel",
        example: `Trích xuất Báo cáo NASA Mars Pathfinder 1997 & FreeRTOS Kernel Book Ch.7: Task Meteo (Mức 1) đang giữ Mutex. Task Bus (Mức 3) yêu cầu Mutex này -> Kế thừa mức ưu tiên: Tạm thời nâng mức ưu tiên của Task Meteo lên mức max(1, 3) = 3 cho đến khi nhả Mutex, bảo vệ hệ thống không bị Task Comm (Mức 2) chen ngang.`,
        hint: `Nếu task_prio < waiting_task_prio thì trả về waiting_task_prio; ngược lại trả về task_prio.`,
    ...
    },
    {
        ...
        docSnippet: "FreeRTOS Kernel Book Ch.3: 'Priority Scheduler scans ready lists from the highest priority to find the next task to run.'",
        standardRef: "FreeRTOS Kernel Book Ch.3 / Priority-Based Preemptive Scheduler",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS Kernel Book Ch.3: 3 tác vụ trong hàng đợi sẵn sàng có mức ưu tiên p0 = 1, p1 = 4, p2 = 2 -> Tác vụ có mức ưu tiên cao nhất là Task 1 với mức ưu tiên 4 -> Bộ lập lịch trao CPU cho Task 1.`,
        hint: `Tìm giá trị lớn nhất trong các mức ưu tiên.`,
    ...
    },
    {
        ...
        docSnippet: "MLPerf Tiny Benchmark: 'Multi-core balance is assessed by computing the absolute delta in processor utilization between Symmetric Multi-Processing cores.'",
        standardRef: "MLPerf Tiny Benchmark / Dual-Core SMP Load Balancing",
        bookId: "book_freertos_kernel",
        example: `Trích xuất FreeRTOS SMP & MLPerf Tiny: Nhân 0 chạy 90% tải, Nhân 1 chạy 20% tải. Chênh lệch độ lệch tải = |90 - 20| = 70% vượt ngưỡng cảnh báo 50% -> Bật cờ cảnh báo mất cân bằng tải (Output = 1).`,
        hint: `Kiểm tra: <code>abs(load_core0 - load_core1) > threshold ? 1 : 0</code>.`,
    ...
    // =========================================================================
    {
        ...
        docSnippet: "ESP-IDF OTA Guide: 'The 2-slot partition table toggles between ota_0 (slot 0) and ota_1 (slot 1). Next boot partition is flipped after successful validation.'",
        standardRef: "ESP32-S3 TRM Ch.2 / Dual-OTA Partition Scheme",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP32-S3 TRM Ch.2 & ESP-IDF OTA: Phân vùng đang chạy là current_slot = 0 (ota_0). Nạp bản cập nhật mới vào slot tiếp theo -> next_slot = 1 - current_slot = 1 (ota_1).`,
        hint: `Công thức: <code>current_slot == 0 ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "RFC 9000 §5.4: 'XOR checksum verification provides lightweight corruption detection for telemetry and firmware chunk transfer.'",
        standardRef: "SEI CERT C / RFC 9000 / XOR Packet Checksum",
        bookId: "book_sei_cert_c",
        example: `Trích xuất SEI CERT C & RFC 9000: 3 byte dữ liệu firmware nhận được là [0xAA, 0x55, 0xFF]. Mã kiểm tra XOR = 0xAA ^ 0x55 ^ 0xFF = 0x00.`,
        hint: `Duyệt mảng và XOR dồn tất cả các phần tử.`,
    ...
    },
    {
        ...
        docSnippet: "IEEE 802.11 Standards: 'RSSI values below -75 dBm experience sharp packet loss increases, endangering continuous OTA flashing streams.'",
        standardRef: "IEEE 802.11 Wi-Fi Standards / ESP32-S3 TRM Radio",
        bookId: "book_esps3_trm",
        example: `Trích xuất IEEE 802.11 & ESP32-S3 TRM: Cường độ sóng đo được rssi = -68 dBm, ngưỡng an toàn tối thiểu threshold = -75 dBm. Vì -68 > -75 (sóng đủ mạnh) -> Cho phép tiến hành nạp OTA (Output = 1).`,
        hint: `Kiểm tra: <code>rssi >= threshold ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "IEEE 802.11 Signal Quality: 'Signal strength representation maps RSSI (dBm) to 0-4 UI bars: >= -60 (4 bars), >= -75 (3 bars), >= -85 (2 bars), >= -95 (1 bar).'",
        standardRef: "IEEE 802.11 / Wi-Fi Signal Bars Representation",
        bookId: "book_esps3_trm",
        example: `Trích xuất IEEE 802.11: Cường độ rssi = -55 dBm (rất mạnh, >= -60) -> 4 vạch; rssi = -72 dBm (>= -75) -> 3 vạch; rssi = -82 dBm (>= -85) -> 2 vạch; rssi = -90 dBm -> 1 vạch; yếu hơn -> 0 vạch.`,
        hint: `Phân nhánh theo các mốc: -60, -75, -85.`,
    ...
    },
    {
        ...
        docSnippet: "RFC 9000 §6.2.1: 'Exponential Backoff: retry_delay = min(max_delay, base_delay * (2 ^ retry_count)) prevents congestion collapse during reconnection.'",
        standardRef: "RFC 9000 / Exponential Backoff Network Reconnect",
        bookId: "book_sei_cert_c",
        example: `Trích xuất RFC 9000 & AWS IoT Best Practices: Thời gian gốc base_delay = 1 giây, giới hạn max_delay = 30 giây. Thử lại lần thứ retry = 3 -> Thời gian chờ = 1 * (2 ^ 3) = 8 giây.`,
        hint: `Công thức: <code>Math.min(max_delay, base_delay * (1 << retry))</code>.`,
    ...
    },
    {
        ...
        docSnippet: "OASIS MQTT v5.0: 'Compact binary payloads maximize throughput on constrained networks. Multiple telemetry metrics are packed into single 32-bit registers.'",
        standardRef: "OASIS MQTT v5.0 Standard / MISRA C:2012 Rule 10.1 Bit Packing",
        bookId: "book_misra_c",
        example: `Trích xuất OASIS MQTT v5.0 & MISRA C:2012 Rule 10.1: Mã thiết bị dev_id = 0x1234 (16-bit), mã lỗi err_code = 0x05 (8-bit), mức pin battery = 98 (8-bit) -> Đóng gói thành từ 32-bit duy nhất: (0x1234 << 16) | (0x05 << 8) | 98.`,
        hint: `Công thức: <code>(dev_id << 16) | (err_code << 8) | battery</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ISO C11 §6.5: 'Bitwise shift and mask expressions extract packed telemetry sub-fields without dynamic JSON deserialization.'",
        standardRef: "ISO/IEC 9899:2011 §6.5 Expressions / Bit Unpacking",
        bookId: "book_iso_c11_standard",
        example: `Trích xuất ISO/IEC 9899:2011 §6.5: Nhận gói packet = 0x12340562. Trích xuất Device ID ở 16 bit cao -> (0x12340562 >> 16) & 0xFFFF = 0x1234 = 4660.`,
        hint: `Công thức: <code>(packet >> 16) & 0xFFFF</code>.`,
    ...
    },
    {
        ...
        docSnippet: "MISRA C:2012 Rule 12.2: 'Shift operators on unsigned operands ensure exact sub-field bit extraction without sign extension.'",
        standardRef: "MISRA C:2012 Rule 12.2 / Bitwise Data Extraction",
        bookId: "book_misra_c",
        example: `Trích xuất MISRA C:2012 Rule 12.2: Nhiệt độ nằm ở 8 bit giữa (bit 8 đến 15) của gói tin packet = 0x12341E62 -> (0x12341E62 >> 8) & 0xFF = 0x1E = 30 độ C.`,
        hint: `Công thức: <code>(packet >> 8) & 0xFF</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP-IDF OTA Progress Callback: 'Percentage completion = (bytes_written * 100) / total_image_size.'",
        standardRef: "ESP-IDF OTA Progress Callback Architecture",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP-IDF OTA Progress Architecture: Đã ghi được bytes_written = 250,000 bytes, tổng dung lượng firmware total_bytes = 1,000,000 bytes -> Tiến độ = (250,000 * 100) / 1,000,000 = 25%.`,
        hint: `Công thức: <code>(bytes_written * 100) / total_bytes</code>.`,
    ...
    },
    {
        ...
        docSnippet: "RFC 791: 'MTU defines the maximum size of a packet that can be transmitted without fragmentation. Standard Ethernet MTU is 1500 bytes.'",
        standardRef: "RFC 791 / RFC 8200 Maximum Transmission Unit (MTU)",
        bookId: "book_sei_cert_c",
        example: `Trích xuất RFC 791 & RFC 8200: Giới hạn mtu = 1500 bytes. Gói tin truyền đi packet_size = 1520 bytes. Vì 1520 > 1500 -> Vi phạm giới hạn MTU (Output = 1), cần chia nhỏ gói tin.`,
        hint: `Kiểm tra: <code>packet_size > mtu ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ISO 11898-1 / Dallas Maxim: 'CRC-8 with generator polynomial x^8 + x^2 + x + 1 (0x8C / 0x07 reversed) verifies frame integrity in noisy automotive and industrial environments.'",
        standardRef: "ISO 11898-1 CAN Bus / Dallas-Maxim CRC-8 Polynomial",
        bookId: "book_can_iso11898",
        example: `Trích xuất ISO 11898-1 CAN Bus: Dữ liệu data = 0x31 (ký tự '1'). Tính theo thuật toán dịch bit CRC-8 -> Cho ra mã kiểm tra tương ứng để gắn vào đuôi gói tin.`,
        hint: `Duyệt qua 8 bit của dữ liệu và XOR với đa thức đa thức khi bit cao bằng 1.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §2.3: 'The first byte of any valid ESP32 application binary image must be the Magic Byte 0xE7. The ROM bootloader rejects images with any other byte.'",
        standardRef: "ESP32-S3 TRM Ch.2 Bootloader & ROM Image Format",
        bookId: "book_esps3_trm",
        example: `Trích xuất ESP32-S3 TRM Ch.2: Byte đầu tiên của file nạp là header_byte = 0xE7 -> Output = 1 (TRUE - File firmware chuẩn của ESP32-S3). Nếu là 0x00 -> Output = 0 (Từ chối nạp).`,
        hint: `Kiểm tra: <code>header_byte == 0xE7 ? 1 : 0</code>.`,
    ...
    // =========================================================================
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.7: 'A confidence score threshold (e.g., 85%) prevents false activations. Predictions below this threshold are suppressed as background noise.'",
        standardRef: "TinyML (O'Reilly) Ch.7 Wake-Word Detection / Confidence Filter",
        bookId: "book_tinyml_oreilly",
        example: `Trích xuất TinyML (O'Reilly) Ch.7: Điểm tin cậy của mô hình nhận diện giọng nói là score = 88, ngưỡng yêu cầu threshold = 85. Vì 88 >= 85 -> Xác nhận người dùng vừa đọc từ khóa (Output = 1).`,
        hint: `Kiểm tra: <code>score >= threshold ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Jacob et al. 2018 CVPR §2: 'The Rectified Linear Unit activation f(x) = max(0, x) clamps negative activations to zero while passing positive values unchanged.'",
        standardRef: "Benoit Jacob et al. 2018 CVPR Section 2 / ReLU Activation",
        bookId: "book_jacob_quantization",
        example: `Trích xuất Benoit Jacob et al. 2018 CVPR Section 2: Giá trị đầu vào x = -15 -> f(x) = 0 (Triệt tiêu tín hiệu âm); Đầu vào x = 42 -> f(x) = 42.`,
        hint: `Nếu x < 0 trả về 0; ngược lại trả về x.`,
    ...
    },
    {
        ...
        docSnippet: "Jacob et al. 2018 CVPR Eq. 1: 'Quantized representations are constrained to a signed 8-bit container: q in [-128, 127]. Overflow values must be clamped to avoid wrap-around distortion.'",
        standardRef: "Benoit Jacob et al. 2018 CVPR Eq. 1 / Quantized Range Clamping",
        bookId: "book_jacob_quantization",
        example: `Trích xuất Benoit Jacob et al. 2018 CVPR Eq. 1: Giá trị tính toán val = 145 (Vượt quá giới hạn 127 của int8_t) -> Cắt ngọn về 127. Nếu val = -130 -> Cắt ngọn về -128.`,
        hint: `Nếu val > 127 trả về 127; nếu val < -128 trả về -128; ngược lại trả về val.`,
    ...
    },
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.11: 'Binary decision thresholding compares output probability against 50% to trigger physical actuators (relays, alarms).'",
        standardRef: "TinyML (O'Reilly) Ch.11 Vision Person Detection / Binary Threshold",
        bookId: "book_tinyml_oreilly",
        example: `Trích xuất TinyML (O'Reilly) Ch.11: Xác suất có người probability = 72%, ngưỡng quyết định threshold = 50% -> Output = 1 (TRUE - Có người xuất hiện, bật đèn chiếu sáng).`,
        hint: `Kiểm tra: <code>probability >= threshold ? 1 : 0</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Jacob et al. 2018 CVPR §2: 'Real value r is mapped to quantized integer q: q = round(r / S) + Z, where S is the Scale factor and Z is the Zero-point integer.'",
        standardRef: "Benoit Jacob 2018 CVPR Section 2 / Affine Quantization Scheme",
        bookId: "book_jacob_quantization",
        example: `Trích xuất Benoit Jacob et al. 2018 CVPR Section 2: Cho giá trị thực r = 0.5f, tỉ lệ scale = 0.05f, điểm zero_point = 0 -> Giá trị số nguyên INT8 = round(0.5 / 0.05) + 0 = 10. Dung lượng lưu trữ giảm 4 lần!`,
        hint: `Công thức: <code>(int)Math.round(r / scale) + zero_point</code>.`,
    ...
    },
    {
        ...
        docSnippet: "Jacob et al. 2018 CVPR §2: 'The dequantization operator reconstructs the continuous real value: r = S * (q - Z).'",
        standardRef: "Benoit Jacob et al. 2018 CVPR / Dequantization Scheme",
        bookId: "book_jacob_quantization",
        example: `Trích xuất Benoit Jacob et al. 2018 CVPR Section 2: Giá trị nơ-ron số nguyên q = 10, scale = 0.05f, zero_point = 0 -> Giá trị số thực r = 0.05 * (10 - 0) = 0.5 (hoặc 50%).`,
        hint: `Công thức: <code>scale * (q - zero_point)</code>.`,
    ...
    },
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.3: 'A 4-dimensional image tensor [B, H, W, C] contains B * H * W * C scalar elements. Memory size in bytes = elements * sizeof(element_type).'",
        standardRef: "TFLite Micro Tensor Specs / TinyML (O'Reilly) Ch.3",
        bookId: "book_tinyml_oreilly",
        example: `Trích xuất TFLite Micro Specs & TinyML (O'Reilly) Ch.3: Kích thước chiều cao h = 28, chiều rộng w = 28, số kênh màu c = 1 -> Tổng số phần tử = 28 * 28 * 1 = 784 phần tử (tương ứng 784 bytes với mô hình INT8).`,
        hint: `Công thức: <code>h * w * c</code>.`,
    ...
    },
    {
        ...
        docSnippet: "MLPerf Tiny (NeurIPS 2021) §3: 'MACs (Multiply-Accumulate operations) for a 1D Convolution: MACs = Output_Length * Kernel_Size * In_Channels * Out_Channels.'",
        standardRef: "MLPerf Tiny Benchmark (NeurIPS 2021) / Conv1D MACs Complexity",
        bookId: "book_mlperf_tiny",
        example: `Trích xuất MLPerf Tiny Benchmark (Banbury et al., NeurIPS 2021): Chuỗi đầu ra out_len = 100, kích thước nhân lọc kernel_size = 3, số kênh vào in_ch = 1, số kênh ra out_ch = 8 -> Tổng số phép tính MACs = 100 * 3 * 1 * 8 = 2,400 phép tính.`,
        hint: `Công thức: <code>out_len * kernel_size * in_ch * out_ch</code>.`,
    ...
    },
    {
        ...
        docSnippet: "ESP32-S3 TRM §1.3: 'The EE.VMUL.S16 instruction computes parallel 16-bit element-wise products and accumulates results into a 32-bit vector register in a single cycle.'",
        standardRef: "ESP32-S3 TRM Ch.1 Vector SIMD / 1D Dot Product",
        bookId: "book_jacob_quantization",
        example: `Trích xuất ESP32-S3 TRM Ch.1 & Jacob 2018: Đầu vào x = [1, 2, 3], trọng số w = [4, 5, 6] -> Dot Product = 1*4 + 2*5 + 3*6 = 4 + 10 + 18 = 32.`,
        hint: `Công thức: <code>x0*w0 + x1*w1 + x2*w2</code>.`,
    ...
    },
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.8: 'The classification decision is obtained via the ArgMax operator, selecting the class index with the highest probability score from the Softmax output array.'",
        standardRef: "TinyML (O'Reilly) Ch.8 / Post-Processing ArgMax",
        bookId: "book_tinyml_oreilly",
        example: `Trích xuất TinyML (O'Reilly) Ch.8: Mảng xác suất Softmax có 4 nhãn [10, 85, 3, 2] (tương ứng: [0='Silence', 1='Yes', 2='No', 3='Unknown']) -> Chỉ số có xác suất lớn nhất là 1 (Phát hiện từ khóa 'Yes' với độ tin cậy 85%).`,
        hint: `Duyệt mảng và tìm chỉ số có giá trị lớn nhất.`,
    ...
    },
    {
        ...
        docSnippet: "Jacob et al. 2018 CVPR §3: 'Real multiplier M is represented as M = 2^(-n) * M_0, where M_0 is an integer in [2^30, 2^31). Real scaling is replaced by fixed-point integer multiplication and bit shifts.'",
        standardRef: "Benoit Jacob et al. 2018 CVPR Section 3 / Quantized GEMM",
        bookId: "book_jacob_quantization",
        example: `Trích xuất Benoit Jacob et al. 2018 CVPR Section 3: Tích hai số nguyên x = 10, w = -5 -> Tích tích lũy = -50. Nhân với hệ số tỷ lệ multiplier = 2 và dịch bit shift = 1 -> Output = ((-50) * 2) >> 1 = -50. Thực thi 100% bằng số nguyên không cần FPU.`,
        hint: `Công thức: <code>((x * w) * multiplier) >> shift</code>.`,
    ...
    },
    {
        ...
        docSnippet: "TinyML (O'Reilly) Ch.12: 'AutoEncoder Anomaly Detection calculates the Mean Absolute Error (MAE) between input x and reconstructed output x_hat: Error = Sum(|x_i - x_hat_i|).'",
        standardRef: "TinyML (O'Reilly) Ch.12 & MLPerf Tiny ToyADMOS / Anomaly Detection",
        bookId: "book_tinyml_oreilly",
        example: `Trích xuất TinyML (O'Reilly) Ch.12 & MLPerf Tiny ToyADMOS: Tín hiệu rung đầu vào x = [10, 20, 30], tín hiệu AutoEncoder tái tạo x_hat = [12, 19, 35] -> Sai số tái tạo = |10 - 12| + |20 - 19| + |30 - 35| = 2 + 1 + 5 = 8. Nếu vượt quá ngưỡng 15 -> Phát cảnh báo hỏng hóc.`,
        hint: `Tính tổng sai số tuyệt đối: <code>abs(x0 - x_hat0) + abs(x1 - x_hat1) + abs(x2 - x_hat2)</code>.`,
...
function loadProblemDetails(index) {
    ...
    const snippetEl = document.getElementById("prob-doc-snippet");
    const snippetCard = document.getElementById("prob-doc-snippet-card");
    const snippetSource = document.getElementById("prob-doc-source-tag");
    if (snippetCard) {
        if (prob.docSnippet) {
            snippetCard.style.display = "block";
            if (snippetEl) snippetEl.innerHTML = prob.docSnippet;
            if (snippetSource) snippetSource.innerText = prob.standardRef || "Tài Liệu Gốc";
        } else {
